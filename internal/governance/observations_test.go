...
func TestQueryObservationsStableScopeAcrossRevisionsAndStates(t *testing.T) {
	...
	assertTPMTotals(t, first.ScopeTotals.TPM, "16", "2", "1", "1", "1", "1")
	assertCostTotals(t, first.ScopeTotals.Cost, "3", "1", "1", "1", "1", map[string][2]string{
		"EUR": {"30", "1"}, "USD": {"55", "2"},
	})
...
func TestQueryObservationsPaginationStrictCursorAndHardOnlySnapshot(t *testing.T) {
	...
	assertTPMTotals(t, first.Items[0].ScopeTotals.TPM, "6", "3", "0", "0", "0", "0")
...
func TestObservationMultiScopeLateSettlementAndRecovery(t *testing.T) {
	t.Run("multi-scope", func(t *testing.T) {
		...
		assertTPMTotals(t, employeeItem.ScopeTotals.TPM, "2", "1", "0", "0", "0", "0")
		...
		assertTPMTotals(t, groupItems[0].ScopeTotals.TPM, "5", "2", "0", "0", "0", "0")
	})
	...
	t.Run("late-page-settlement", func(t *testing.T) {
		...
		assertTPMTotals(t, first.Items[0].ScopeTotals.TPM, "2", "1", "0", "1", "0", "0")
		...
		assertTPMTotals(t, second.Items[0].ScopeTotals.TPM, "5", "2", "0", "0", "0", "0")
	})
	...
	t.Run("recovery-and-idempotent-finish", func(t *testing.T) {
		...
		for _, item := range page.Items {
			if item.Snapshot.PolicyID == "a-recovery" {
				assertTPMTotals(t, item.ScopeTotals.TPM, "4", "1", "1", "0", "0", "0")
			}
		}
	})
}
...
func assertSameObservationTotals(t *testing.T, left, right ObservationScopeTotals) {
	...
	if left.TPM != right.TPM || len(left.Cost.ByCurrency) != len(right.Cost.ByCurrency) ||
		...
		left.Cost.PendingAttempts != right.Cost.PendingAttempts || left.Cost.PendingRequestsWithoutAttempt != right.Cost.PendingRequestsWithoutAttempt ||
...
func assertTPMTotals(t *testing.T, got ObservationTPMTotals, tokens, known, unknown, pending, pendingNoAttempt, zero string) {
	t.Helper()
	if got.KnownTokens != tokens || got.KnownAttempts != known || got.UnknownTokenAttempts != unknown || got.PendingAttempts != pending ||
		got.PendingRequestsWithoutAttempt != pendingNoAttempt || got.ZeroAttemptRequests != zero {
		t.Fatalf("TPM totals=%+v", got)
	}
}

func assertCostTotals(t *testing.T, got ObservationCostTotals, known, unknown, pending, pendingNoAttempt, zero string, currencies map[string][2]string) {
	t.Helper()
	if got.KnownAttempts != known || got.UnknownCostAttempts != unknown || got.PendingAttempts != pending ||
