...
func newKeyPolicyAdminFixture(t *testing.T) *keyPolicyAdminFixture {
	...
	for _, statement := range []string{
		`CREATE TABLE access_key_policy_sources(key_id TEXT PRIMARY KEY NOT NULL REFERENCES access_key_policies(key_id) ON DELETE CASCADE,source_mode TEXT NOT NULL CHECK(source_mode IN ('all','selected')))`,
		`CREATE TABLE access_key_policy_source_cidrs(key_id TEXT NOT NULL REFERENCES access_key_policy_sources(key_id) ON DELETE CASCADE,cidr TEXT NOT NULL,PRIMARY KEY(key_id,cidr))`,
	} {
