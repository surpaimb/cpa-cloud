...
func (a *App) legacyEmployeeRoute(ctx context.Context, auth employeeAuth, model string, providers []string) (route, *modelAdmissionError) {
	...
	var selected route
	err = a.store.db.QueryRowContext(ctx, `SELECT u.id,u.endpoint,m.upstream_model,m.wire_protocol,u.credential_ciphertext,u.provider_kind,u.revision,u.credential_state,u.key_version FROM models m JOIN upstreams u ON u.id=m.upstream_id WHERE m.id=? AND m.enabled=1 AND m.archived=0 AND u.enabled=1 AND u.archived=0
		AND NOT EXISTS(SELECT 1 FROM account_recovery_states recovery WHERE recovery.account_id=u.id)`, model).Scan(&selected.AccountID, &selected.Endpoint, &selected.UpstreamModel, &selected.WireProtocol, &selected.Ciphertext, &selected.ProviderKind, &selected.Revision, &selected.CredentialState, &selected.KeyVersion)
	if errors.Is(err, sql.ErrNoRows) {
