...
- 根在最终代码 `1970b8b9cc9f655781506f0ea50b56d055b882bd` 上完成本机回归：`go test ./... -count=1 -timeout 15m` 全部通过（service 690.479s，governance 134.732s，accounting 85.808s，backup 44.480s，financial 18.993s，两个 CLI 及其余包均通过），`go vet ./...` 通过；网页 TypeScript、17 文件/122 项测试和生产构建通过。最终程序通过 Responses 同步 smoke，以及生命周期、定时任务和加密备份组合进程 smoke；只使用临时目录、随机回环端口和合成凭据，未访问 8787 或真实提供商。
- Playwright 桌面与 390×844 移动验收覆盖 selector-aware 通用预算和自动备份的入口、默认关闭/未 ready/空历史状态、表单、导航与窄屏布局；两页均无横向溢出或可见控件截断。唯一 console error 是认证前 session 探测的预期 401，登录后交互没有页面异常。商业流程目前没有网页入口，本批只按 API/service 测试验收。
