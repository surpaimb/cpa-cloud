...
状态：本地源码集成验收通过，Linux CI 待验证。本文描述本批底层实现，不代表自动生成恢复已启用。整个功能仍按
[执行计划](account-recovery-execution-plan.md)推进；后续后台策略、开关、失败快照采集和网页入口尚未接通。
...
本批没有创建探测的 HTTP 接口、CLI 开关或定时 worker；正常使用不会自动发送新的生成请求。
...
仍需补齐的环节：默认关闭的启用配置、真实失败路径快照、保留较长冷却原因时的快照一致性、后台有界调度、
持久退避和新 operation 分配、Codex 刷新 revision 的可证明采纳、网页状态与控制。这些项保留在总目标中。
后续接口与失败快照约定见[后台协调器实施契约](account-recovery-coordinator-contract.md)。当前 receipt
覆盖明确回滚后的元数据重试；“实际提交成功但 Commit 返回错误”的在线终态解析仍待该批实现，
当前此种结果会保守占用容量至 TTL/重启，不再次调用上游。
