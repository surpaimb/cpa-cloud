...
HTTP 用量/故障专项及完整 Go 回归曾通过。当前源码另接入账号池预检换号，本轮隔离进程、完整 Go 及 Linux race 已通过，未发布新安装包。价格版本、管理员统计和网页已按
...
`failover` attempt。测试不访问真实上游、不读取真实凭据或真实请求内容；实际运行证据见[集成状态](integration-status.md)。
