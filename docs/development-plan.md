...
2026-09-25 的当前批次由[流式与 Key 策略批次契约](stream-key-policy-batch-contract-2026-09-25.md)约束：只把既有 Chat↔Responses 文本/function SSE 转换接入显式 wire 真实执行链，并实现每 Key 客户端协议与公开模型策略、CAS 管理接口和网页。六个非流式显式 wire 方向已在 `main`；Messages/Gemini 跨协议 SSE、媒体、托管工具、有状态/后台转换、IP/CIDR、组策略、通用 TPM 上界、自助门户和真实支付均不在本批。
...
治理已接 RPM/并发及有限范围固定模型预算，Token/成本未知值不当作零；通用 TPM/成本硬上界及其余矩阵需求继续保留。测试夹具优化独立审阅，
