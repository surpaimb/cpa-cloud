...
2026-09-24；**本轮预算核心、管理配置、网页及请求执行已通过独立验收，并经 [PR #1](https://github.com/surpaimb/cpa-cloud/pull/1) 合入主线 `d95b6a7`。**
测试源码为 `ac4d820`，合并没有改变该源码的实现。范围仍是默认关闭、固定模型的源码实验。
...
- 网页预算专项 11 项、网页全量 109 项测试通过，typecheck/build 通过。
...
- 根在 `ac4d820` 的真实 Go 程序和生产网页构建上完成 Playwright 验收：随机回环端口、合成员工/密码及独立临时数据。
  桌面 1440×1000、手机 390×844 均可操作；创建、编辑、两个开关独立保存及重载后保留通过。
  成本字符串 `9007199254740993` 原样保存，未被 JavaScript 安全整数边界截断；policy revision 1→2、settings revision 1→4。
  页面标题正确、内容非空、无错误覆盖层；0 页面异常、0 非预期 HTTP 失败，只有匿名会话预期的 401。
  已查看桌面、手机和手机编辑框截图；表格在窄屏可横向滚动，保存按钮可见且实际点击成功。
  截图在本机 `C:/TopC9-QA/cpa-budget-qa-20260924/`，未加入仓库；程序、浏览器及临时数据均已清理。

全部为合成数据自动化；浏览器验收未发送模型请求。未验证真实提供商调用。
不发布安装包，不据此宣称已完整对齐 Sub2API 预算机制。

## Linux CI 与本轮交付

[Code validation 35927456340](https://github.com/surpaimb/cpa-cloud/actions/runs/35927456340) 全部通过；根读取实际 job 日志核实：

- core `107405872942`：普通 Linux service 132.048 秒；race service 1323.013 秒、membership 1.870 秒、scheduling 1.086 秒、
  accounting 59.363 秒、egress 1.319 秒、governance 8.858 秒均 PASS。随后全仓 vet、程序编译和治理/观测/代理存储/
  上游健康/恢复基础/自动恢复六组隔离进程验收均 PASS。
- web `107405872936`：typecheck、13 文件/109 项测试与生产构建通过；Windows/Linux/macOS 安装任务均 skipped。
- 本次合并的运行时代码与通过 PR 检查的 `ac4d820` 完全一致。双语文档另行记录验收结果和受支持请求示例，
  不改动已验证的程序及网页实现；未新增 tag 或安装包。
