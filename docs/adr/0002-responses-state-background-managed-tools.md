# ADR 0002：Responses 状态、后台任务与托管工具安全边界
...
D1 已实现无状态 Chat↔Responses 转换模块和严格流状态机；本 ADR 本身不启用任何状态、后台或托管工具。
D2 必须先经迁移和恢复审阅，再接 App/worker。D3 必须逐工具完成白名单契约和官方上游合成测试。真实
供应商与真实客户端证据单独报告。
