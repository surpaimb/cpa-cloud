...
可选请求头 `X-CPA-Session` 接受 1–256 字节会话标识；仅使用绑定员工、Key、模型和协议的 HMAC 做短时粘滞，不保存或转发原值。租约会续期、释放并在重启后保守恢复。显式账号池中，凭据解密等账号预检失败且模型请求尚未派发时，最多切换一个不同账号，并重新检查权限和池版本；账本使用实际发送账号及模型的价格。**进入上游 HTTP 调用或 Codex 执行器后不自动重放**，包括连接错误、429 和流式错误。429、认证或暂时故障的冷却影响后续独立请求。恢复探测、出站代理池、配额采集和员工预算尚未实现。见 [运行时契约](docs/account-pool-runtime-contract.md)和[安全换号契约](docs/account-pool-failover-contract.md)。以上为源码功能，不包含在 preview.3 下载包中。
...
这些管理操作仅接受管理员会话与 CSRF 校验，员工 Key 不可调用。自动生成恢复探测仍未实现。接口与失败语义见[上游测试与恢复契约](docs/upstream-health-contract.md)。

最新源码已加入生成恢复的基础模块：独立系统探测账本、与员工请求共用容量的维护租约、持久化恢复隔离，以及四种 API 协议和 Codex 的固定输入探测执行器。管理员可读取 `GET /admin/api/v1/system-probes/summary`，探测用量不混入员工统计，未知用量和成本保持为空。目前没有启动探测的网页入口、命令行开关或后台定时任务，正常运行不会自动生成探测请求。详见[基础模块与未完成项](docs/system-probe-foundations-contract.md)；这些改动尚未包含在 preview.3 下载包中。
...
页面显示的是内部**估算成本**，不是供应商账单或向员工收费。部分 Token 未知时不会编造完整成本；未定价、未知用量的次数单独显示。余额、售价、预算执行、日/月报表导出与支付仍待实现。接口和精度规则见 [用量与价格契约](docs/usage-management-contract.md)。
