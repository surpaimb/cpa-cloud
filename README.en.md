...
The optional `X-CPA-Session` header accepts a 1–256-byte identifier. Only an employee/key/model/protocol-scoped HMAC is used for short-lived affinity; the original value is neither stored nor forwarded. Leases renew, release, and conservatively recover across restarts. In an explicit account pool, an account-specific preparation failure such as credential decryption can select one different account before model dispatch, with fresh authorization and pool revision checks. Accounting uses the account and model actually dispatched. **No automatic replay is allowed after entering the upstream HTTP call or Codex executor**, including connection errors, HTTP 429, and streaming errors. Rate limits, authentication errors, and temporary failures cool accounts for later independent requests. Opt-in recovery probes are described below; the first outbound-proxy batch is described below; upstream quota collection and general employee budget coverage remain unfinished; the fixed-model hard-budget experiment is described below. See the [runtime contract](docs/account-pool-runtime-contract.md) and [safe failover contract](docs/account-pool-failover-contract.md). These source features are not included in preview.3 downloads.
