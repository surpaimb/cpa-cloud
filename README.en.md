...
- Stateless remains the default. Source builds can enable employee/key-owned encrypted `store:true`, `previous_response_id`, and `GET/DELETE /v1/responses/{id}` with `--responses-stateful-resources`; `--responses-background-tasks` must also be explicit before creation, polling, and cancellation are available. A crash after durable dispatch becomes `interrupted` and is never automatically replayed.
- Background input is restricted to the fully persisted text, instructions, and function-result subset; other fields are rejected before enqueue. The hosted-tool whitelist remains empty, `managed_tools=false`, and there is no hidden switch. Background stream resume/cursors and WebSocket are unavailable. Codex image, audio, and unsupported fields are explicitly rejected.
...
The optional `X-CPA-Session` header accepts a 1–256-byte identifier. Only an employee/key/model/protocol-scoped HMAC is used for short-lived affinity; the original value is neither stored nor forwarded. Leases renew, release, and conservatively recover across restarts. In an explicit account pool, an account-specific preparation failure such as credential decryption can select one different account before model dispatch, with fresh authorization and pool revision checks. Accounting uses the account and model actually dispatched. **No automatic replay is allowed after entering the upstream HTTP call or Codex executor**, including connection errors, HTTP 429, and streaming errors. Rate limits, authentication errors, and temporary failures cool accounts for later independent requests. Opt-in recovery probes are described below; the first outbound-proxy batch is described below; provider quota collection remains unfinished; selector-aware general budgets and the fixed-model hard-budget proof are described below. See the [runtime contract](docs/account-pool-runtime-contract.md) and [safe failover contract](docs/account-pool-failover-contract.md). These source features are not included in preview.3 downloads.
...
The latest source also provides `/admin/api/v1/budgets` and a selector-aware budget page. Employee, key, and governance-group scopes can be narrowed by protocol and public model; all matching policies combine to the strictest result. The policy/window framework is general, but the safe bound proof still covers only a fixed parameter shape for non-streaming text requests to official OpenAI `gpt-4.1-2025-04-14`. Other models, tools, membership requests and SSE are rejected under strict `deny_unknown`. Cost limits require a matching-currency price. Unknown usage keeps the conservative bound instead of becoming zero; this is not a provider invoice guarantee. See [integration evidence and remaining scope](docs/budget-service-integration-progress.md). Preview.3 downloads do not include this work. Tenant limits are out of this increment.
...
The public scope permits public proxy addresses; the private scope permits RFC1918/IPv6 ULA proxy addresses without relaxing model-target restrictions. Both TLS layers verify certificates, and target DNS addresses are checked on every dial. HTTP/SOCKS, automatic proxy rotation, exit-IP lookup and Codex bindings are not implemented in this batch.

A disabled, invalid or undecryptable proxy never silently falls back to a direct connection. Explicitly unbind the account to restore direct access. Address, authentication and enabled-state changes increment the connection revision and every bound account revision. Queued requests cannot switch to the new connection implicitly; dispatched requests may finish on their original client. Proxy edits do not clear account cooldown or recovery isolation.

**Handshake only** establishes CONNECT and target TLS for an explicitly bound HTTPS API-key account. It sends no model request, catalog request or upstream API key. Results retain the tested revisions and do not prove generation readiness. A lost submission response is recovered by querying the original operation; the page does not automatically submit another test. Restarted incomplete operations become interrupted without reconnecting.

This is a source feature, **not included in preview.3 downloads**. Before upgrading, preserve the complete data directory and matching old executable. Roll back using that full backup; do not run an old executable against a new database containing proxy bindings, as it may ignore them and connect directly. See the [management contract](docs/outbound-proxy-admin-contract.md) and [handshake contract](docs/outbound-proxy-test-contract.md).
...
These operations require an administrator session and CSRF validation; employee keys cannot invoke them. See the [upstream health contract](docs/upstream-health-contract.md) for API and failure semantics.

### Automatic account recovery (off by default, latest source only)

Add `--allow-account-recovery` to your existing service startup command, then enable recovery under **System status → Account recovery** in the web console. Both gates must be enabled; the startup flag alone starts no probes. Recovery sends fixed short inputs to your upstream and consumes usage, so configure accounts and prices first. Without the flag, administrators cannot enable recovery, but can disable a previously saved setting.

Failures in explicit account pools capture the account, actual model, protocol and revisions used by the request. After cooldown, one worker probes within shared account capacity. Only a complete generation result matching the current snapshot removes isolation. Each cooldown event permits at most three durable attempts; transient failures wait at least five minutes before a new operation. Installation-wide probe history is capped at 10000 records. Authentication/protocol failures, configuration changes and exhausted limits require administrator review. Cooldown expiry, catalog success and administrator enablement do not bypass recovery isolation.

Disabling recovery cancels the current probe and preserves isolation and history. Uncertain dispatch or settlement never replays the same operation. After losing a settings response, read the current state before retrying. The upstream list can clear cooldown and isolation using the current account revision and event, but clearing does not prove generation availability. After a crash, restored requests retain capacity until their leases expire; clearing isolation does not release that capacity early. Upgraded databases default to disabled; old cooldowns do not imply an invented probe path.

Probe accounting is separate from employee statistics. Administrators can read `GET /admin/api/v1/system-probes/summary`; unknown usage and cost stay null. See the [coordinator contract](docs/account-recovery-coordinator-contract.md) for API, failure and migration details. Acceptance uses synthetic credentials and mock providers; real membership accounts remain unverified. **preview.3 downloads do not include this feature.**
...
These are internal **estimated costs**, not provider invoices or employee charges. The latest source adds reliable dispatch/usage facts, daily and monthly summaries, bounded CSV export, and append-only corrections. Missing token buckets do not produce invented complete costs; unpriced and unknown attempts are counted separately. See the [usage and pricing contract](docs/usage-management-contract.md) for APIs and precision rules.

### Single-instance ledger and commercial flows (off by default)

`/admin/api/v1/billing` provides fixed-point balances and plan snapshots, subscription purchase/cancellation, pending top-ups, redemption codes, and partial/full refunds scoped to an employee, key, or employee-owned resource. Monetary and accepted-operation facts are append-only. Reusing an operation ID is valid only for the same actor, action, and payload. Commercial execution is disabled in the database by default; administrators may prepare plans and encrypted connectors before explicitly enabling it.

The payment callback is a local generic HMAC interface with a five-minute window, constant-time verification, replay protection, and atomic callback/payment/ledger commit. It is **not a production Stripe, Airwallex, QR, or other provider integration**. Automatic renewal, tax, invoices, notifications, and multi-instance coordination are not included. See the [single-instance billing contract](docs/single-instance-billing-contract.md).

### Automated encrypted backup (off by default)

Source builds add `--automated-backups-enabled`, restricted output roots, plans/run history, retention, and restore rehearsal. Only a Windows current-user DPAPI key provider can currently become ready/running. Linux and macOS do not fall back to plaintext key storage. Cross-machine recovery material, object storage, remote replication, and production rotation remain unavailable. The offline password CLI remains supported; see [backup and restore](docs/backup-restore.md).
