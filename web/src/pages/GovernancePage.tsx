...
function validatePolicy(rpm: string, concurrency: string, tpm: string, cost: string, currency: string): string | null {
  if ([rpm, concurrency, tpm].some((value) => value !== '' && !validPositiveInteger(value))) return 'RPM、并发和 shadow TPM 必须是 1 到 9007199254740991 的整数。'
  if (cost !== '') {
    if (!/^[1-9][0-9]*$/.test(cost) || BigInt(cost) > 9223372036854775807n) return '成本阈值必须是规范十进制正整数字符串，且不超过 9223372036854775807。'
    if (!/^[A-Z]{3}$/.test(currency)) return '成本阈值需要三个大写 ASCII 字母的币种标识。'
  } else if (currency !== '') return '未设置成本阈值时不能设置币种。'
  if ([rpm, concurrency, tpm, cost].every((value) => value === '')) return '至少配置一个 hard 或 shadow 阈值。'
  return null
}

function policyInput(enabled: boolean, rpm: string, concurrency: string, tpm: string, cost: string, currency: string): GovernancePolicyInput {
  return {
    enabled,
    hard: { rpm: parseLimit(rpm)!, concurrency: parseLimit(concurrency)! },
    shadow: { tpm: parseLimit(tpm)!, cost_micro: cost || null, currency: cost ? currency : null, window: cost ? 'rolling_24h' : null },
  }
}
...
export function GovernancePage({ csrf }: { csrf: string }) {
  ...
  async function toggleSettings() {
    if (!settings) return
    const body = { operation_id: crypto.randomUUID(), expected_revision: settings.revision, enabled: !settings.enabled }
...
function PolicyEditor({ initial, employees, keys, groups, csrf, onClose, onSaved }: { initial: GovernancePolicy | null; employees: Employee[]; keys: TargetKey[]; groups: GovernanceGroup[]; csrf: string; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  async function save() {
    const issue = validatePolicy(rpm, concurrency, tpm, cost, currency)
    if (issue) { setValidation(issue); return }
    if (!scopeID) { setValidation('请选择策略作用对象。'); return }
    setValidation(null)
    const operation_id = crypto.randomUUID()
    const limits = policyInput(enabled, rpm, concurrency, tpm, cost, currency)
  ...
  return <Dialog title={baseline ? '编辑治理策略' : '新建治理策略'} description="RPM 与并发会硬拒绝；TPM 与成本首批只保存 shadow 配置，不展示余额或是否超限。" onClose={onClose} wide closeDisabled={write.busy || write.pending}>
    <form onSubmit={submitHandler(async () => save())}>
      <div className="governance-form-grid">
        ...
        <Field label="成本 shadow 阈值（micro）" hint="规范十进制字符串，保持原值精度；固定 rolling_24h。"><input inputMode="numeric" value={cost} disabled={frozen || Boolean(conflict)} onChange={(event) => setCost(event.target.value)} placeholder="不配置" /></Field>
