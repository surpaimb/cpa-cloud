...
import { api, type DiscoveredModel, type ModelRoute, type Upstream, type WireProtocol } from '../api'
...
function wireOptions(provider?: Upstream['provider_kind']): WireProtocol[] {
	if (provider === 'openai-compatible') return ['legacy-native', 'openai-chat', 'openai-responses']
	if (provider === 'codex-membership') return ['legacy-native', 'openai-responses']
...
function EditModelRoute({ csrf, item, onClose, onDone }: { csrf: string; item: ModelRoute; onClose: () => void; onDone: () => void }) {
	...
	const [wireProtocol, setWireProtocol] = useState<WireProtocol>(item.wire_protocol ?? 'legacy-native')
	...
	return <Dialog title={`修改 ${item.id}`} description="公开模型 ID 不可更名。账号池非空时，请先清空账号池再修改默认目标。" onClose={onClose}>
		...
		{!loading && !error ? <form onSubmit={async (event) => {
			...
			try { await api.updateModel(item.id, { expected_revision: revision, upstream_id: upstreamID, upstream_model: upstreamModel.trim(), wire_protocol: wireProtocol, enabled }, csrf); onDone() }
